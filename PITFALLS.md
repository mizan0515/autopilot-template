...
- Resolved-in: open — template could ship a `Test-SurfaceCarriesIdentity.ps1` that enumerates the project's dashboards / signal JSON / operator HTML and fails on missing `repo_identity`.
...
- Resolved-in: open — template could ship `Verify-Terminology.ps1` + a `TERMINOLOGY.example.md` schema that downstreams populate with their own canonical/forbidden pairs.
...
- Resolved-in: open — template could ship `Verify-EvidenceArtifact.ps1 -Kind png|json|log` as a registry-style helper, so downstreams register validators per-kind rather than re-implementing per-project.
...
- Resolved-in: open — template could ship `Test-GeneratedFilesIgnored.ps1` that enumerates known generators (runner, operator-live writer, compact-status writer), runs each in dry-mode or sandbox, and asserts post-run `git status --porcelain` is empty. A CI gate on that script prevents the one-extension-at-a-time leak class entirely.
